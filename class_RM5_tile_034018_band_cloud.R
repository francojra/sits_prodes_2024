...
cubo <- sits_cube(
  ...
  tiles      = c("034018"), # Tiles/Regiões de ineteresse
...
# # Ver diferenças na quantidade de amostras  -----------------------------------------------------------------------------------------
...
tempdir_r <- "prob_suav_034018_bandas_cloud"
dir.create(tempdir_r, showWarnings = FALSE, recursive = TRUE)
...
map_incerteza <- sits_uncertainty(
  cube = probs_class, # Arquivo do cubo de probabilidades com mosaico
  type = "margin",
  output_dir = tempdir_r,
  memsize = 75,
  multicores = 20,
