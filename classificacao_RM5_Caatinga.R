...
plot(map_incerteza, 
     legend_position = "outside", theme_grey()) # rev = FALSE
...
r_df <- as.data.frame(mapa_incert_final, xy = TRUE)
colnames(r_df) <- c("x", "y", "value")  # ajusta o nome da camada



# Esta função irá atribuir NA a todos os pixels que estão dentro da geometria da máscara
# 
# mapa_com_mascara <- mask(mapa_incert_final, mask_sf, inverse = TRUE)

ggplot(mask_sf) +
  geom_sf(fill = "red", color = "red") 

ggplot(mapa_incert_final) +
  geom_spatraster()
